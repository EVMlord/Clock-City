...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
     ...
     *
     * To learn more about hooks, h to xref:ROOT:extending-contracts.adoc#using-hooks[Using Hooks].
     */
    ...
    ReentrancyGuard
{
    ...
    function mint(uint256 _mintAmount) public payable {
        require(!paused, "Clock City: The contract is paused!");
        // pricing
        require(
            msg.value >= _mintAmount.mul(cost),
            "Clock City: Price not met"
        );
        require(
            _mintAmount <= maxMintAmount,
            string(
                abi.encodePacked(
                    "Clock City: You can only mint ",
    ...
    function wlMint(bytes32[] calldata _merkleProof) public payable {
        require(msg.value >= cost, "Clock City: Price not met");
        // Verify whitelist requirements
        require(
            wlMintEnabled,
            "Clock City: The whitelist sale is not enabled!"
        );
        require(
            !wlClaimed[_msgSender()],
            "Clock City: Address already minted!"
        );
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        require(
            MerkleProof.verify(_merkleProof, merkleRoot, leaf) ||
                isWL[msg.sender],
            "Clock City: Oops! You are not on the WL. You will be able to join the public mint."
        );
    ...
    function addToWL(address[] memory _users) public authorized {
    ...
    function removeFromWL(address[] memory _user) public authorized {
        require(_user.length > 0, "No User");
        for (uint256 i = 0; i < _user.length; i++)
            isWL[_user[i]] = false;
    }

    function _mint(address _receiver) internal nonReentrant {
        // ID and supply limits
        require(lastSupply != 0, "Clock City: No more supply");
        uint256 _index = _getRandom() % lastSupply;
        uint256 nextId = getValue(_index) + 1;
        // Just a check to avoid a clash in ID's 
        require(
            !_exists(nextId), "ID already exists!");
        // Reduce supply
        lastSupply--;
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "Clock City: URI query for nonexistent token"
        );
        ...
        CD storage cD = cd[tokenId];

        return
            CCDO.constructTokenURI(
                tokenId,
                cD.s,
                ownerOf(tokenId),
                cD.d,
    ...
    function setPaused(bool _state) public authorized {
        paused = _state;
    }

    function setMerkleRoot(bytes32 _merkleRoot) public authorized {
        merkleRoot = _merkleRoot;
    }

    function setJewelListMintEnabled(bool _state) public authorized {
    ...
    /// @dev withdraw ETH
    function withdrawETH() public authorized {
    ...
    function generateSeed(address _receiver) internal view returns (address) {
        return
            address(
                uint160(
                    uint256(
